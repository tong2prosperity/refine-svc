...
repo_map = {
    ...
    "Plachta/ASTRAL-quantization": {
        "path":hf_cache_dir + "/astral-quantization/",
    },
    "funasr/campplus": {
        "path": hf_cache_dir + "/funasr-campplus/",
...
def load_custom_model_from_hf_map(repo_id, model_filename="pytorch_model.bin", config_filename=None):
    if repo_id in repo_map:
        if config_filename is None:
            return repo_map[repo_id]["path"] + model_filename
        return repo_map[repo_id]["path"] + model_filename, repo_map[repo_id]["path"] + config_filename
    else:
...
def load_custom_model_from_hf(repo_id, model_filename="pytorch_model.bin", config_filename=None):
    ...
    """
    return load_custom_model_from_hf_map(repo_id, model_filename, config_filename)
